					  ...
					  m_isDisconnected( true )
{
	...
	try
	{
		...
		if ( 0 > socketId )
		{
			...
			streamErrInfo << "Unable to create server socket " << service << ".  Reason: " << strerror( errno );
			strErrorInfo = streamErrInfo.str();
		...
		_LOG(SourceFLInfo, DebugDebug, "Created server socket on service port %s", service.c_str());
...
void TcpSocket::close()
{
	...
	try
	{
		...
		if( 0 == socketId )
		{
			_LOG(SourceFLInfo, DebugInfo, "Socket Id = 0 => Not closing socket");
		...
		_LOG(SourceFLInfo, DebugInfo, "Closing socket");
...
void TcpSocket::connect ()
{
	...
	try
	{
		...
		{	             
			...
			streamErrInfo << "Unable to connect to socket.  Reason: '"<<strerror( errno )<<"'";
			strErrorInfo = streamErrInfo.str();
		...
		if( 0 == select( getSocketId() + 1, & lvReadSet, & lvWriteSet, NULL, & m_connectTimeout ) )
		{
			...
			streamErrInfo << "Connection timed out" ;
			strErrorInfo = streamErrInfo.str();
			...
			FD_ISSET( getSocketId(), & lvWriteSet ) )
		{
...
#endif // WIN32
			{
				...
				streamErrInfo << "Error getting socket status" ;
				strErrorInfo = streamErrInfo.str();
			...
			if ( 0 != lvError )
			{
				...
				streamErrInfo << "Socket status returned error, unable to connect." ;
				strErrorInfo = streamErrInfo.str();
		...
		else
		{
			...
			streamErrInfo <<  "Unable to connect" ;
			strErrorInfo = streamErrInfo.str();
...
TcpSocket::TcpSocket( const int pSocketId, 
					 ...
					 m_isDisconnected(true)
{
	//TA_DEBUG_ASSERT( 0 < pSocketId, "SocketId is zero or less" );
	//TA_DEBUG_ASSERT( 0 != pSocketAddress, "SocketAddress is NULL"  );
	...
	if ( 0 == pSocketAddress )
	{
		...
		streamErrInfo <<  "Invalid socket address";
		strErrorInfo = streamErrInfo.str();
...
void TcpSocket::getIpAddress ( struct in_addr * pAddress, const std::string & pHost ) const
{
...
#endif // !defined( SOLARIS )
	{
		...
		streamErrInfo << "Non-existent host";
		strErrorInfo = streamErrInfo.str();
	...
	if ( lvNumReady != 0)
	{
...
#endif // !defined( WIN32 )
		{
				...
				WSAENOTCONN   == WSAGetLastError() )
			{
				...
				_LOG(SourceFLInfo, DebugInfo, "Socket connection reset => socket closed" );
			...
			if( 0 == lvStatus )
			{
				...
				_LOG(SourceFLInfo, DebugInfo, "No data to be read from socket => socket closed" );
